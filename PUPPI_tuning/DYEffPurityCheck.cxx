...
namespace uhh2examples {
  ...
  class DYEffPurityCheck: public AnalysisModule {
  public:
    
    explicit DYEffPurityCheck(Context & ctx);
  ...
  DYEffPurityCheck::DYEffPurityCheck(Context & ctx){
    ...
    else throw std::runtime_error("DYEffPurityCheck -- undefined argument for 'channel' key in xml file (must be 'muon' or 'elec'): "+channel);
  ...
  bool DYEffPurityCheck::process(Event & event) {
    if(berror){
      std::cout<<" "<<std::endl;
...
//////////////////////////////////////// check eff jets 50-100 GeV ////////////////
//if ( event.event != 138274132 ) return false;
//if ( event.event != 83 ) return false;
//if ( event.event != 650800 ) return false;
/*
if ( event.event != 797 ) return false;
for(const Jet & jet: *event.jets){

   //if ( abs(jet.eta())< 2.5 && jet.pt()>50 && jet.pt()<100){
      std::cout<<" ====================    New Event   ====================" << event.event<< std::endl; 
      std::cout<< "raw jet pt  "<<jet.pt()*jet.JEC_factor_raw() << "    " << "jet pt  "<<jet.pt() << "	" << "jet eta  "<<jet.eta() <<std::endl;
      std::cout<< "charged mult" << jet.chargedMultiplicity()<<"      " << "neutral mult" << jet.neutralMultiplicity() << std::endl;
      std::cout<< "num daughters  "<<jet.numberOfDaughters()<< std::endl;
   //}
...
for(const GenJet & genjet: *event.genjets){
   ...
   //if ( abs(genjet.eta())< 2.5 && genjet.pt()>50 && genjet.pt()<100){
   //if ( abs(genjet.eta())> 2.5 && genjet.pt()>30){
      std::cout<<" ====================    New Event   ====================" << event.event<< std::endl; 
      std::cout<< "GEN jet pt  "<<genjet.pt() << "	" << "jet eta  "<<genjet.eta() <<std::endl;
      std::cout<< "GEN charged mult" << genjet.chargedMultiplicity()<<"      " << "neutral mult" << genjet.neutralMultiplicity() << std::endl;
   //}
...
for(const GenJet & genjet: *event.genjets){
for(const Jet & jet: *event.jets){
if ( abs(genjet.eta())> 2.5 &&  abs(genjet.eta())< 3.0 && genjet.pt()>50 && genjet.pt()< 100){
...
//////////////////////////////////////////////////////////////////////////////////
  ...
  UHH2_REGISTER_ANALYSIS_MODULE(DYEffPurityCheck)
}
